 ...
 * Resend documents for their event webhooks. The payload shape read below
 * (`data.from` / `data.to` / `data.subject` / `data.text`) is Resend's
 * documented inbound event as of when this was written; an inbound schema
 * drifts more easily than an outbound API call, so if the parsed fields come
 * back empty once a real reply arrives, log the raw body and adjust the
...
export async function POST(req: Request) {
  ...
  if (!event.data) return NextResponse.json({ ok: true, ignored: true });
